...
ブラウザにメールアドレスやtokenは保存しません。GitHub PagesからSupabase Edge Functionを呼び、Edge Functionが保存済みデータから本文とWordを作り、画面で選択した先生本人を除く全員へGAS経由で送ります。実際の送信元と返信先は、常にGASをデプロイした管理用Google/Gmailアカウントです。

- 担当表: 確定済みアーカイブから、HTML表とWord担当表を作る。
- 授業報告: 保存済み授業記録から、短い挨拶文とWord授業記録を作る。授業記録画面の「メール送信」から確認して送る。
...
Apps Scriptを更新した場合は、既存のWebアプリを「新しいバージョン」で再デプロイしてください。エディタへ貼り付けただけでは、公開中のWebアプリへHTML本文・Word添付の変更は反映されません。
...
- HTML本文とWord添付はブラウザから受け取らず、Edge Functionが確定済みアーカイブから生成すること。
- GASはWordファイルを1件だけ受け付け、ファイル名・MIME type・容量を検証すること。
- 送信失敗時だけ再試行できること。
